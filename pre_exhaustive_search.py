...
def copy_atoms(path, prefix,start_xtal_num, end_xtal_num, new_ground_structure_path, atoms_new, out_dir):
    ...
    for xtal_name in xtals:
        ...
        if os.path.exists(os.path.join(path,xtal_name,"refine.split.ground-state.pdb")):
            ...
            os.system('cp -rL {} {}'.format(os.path.join(path,xtal_name,"refine.mtz"),
            ...
            cmds = "source /dls/science/groups/i04-1/software/pandda-update/ccp4/ccp4-7.0/setup-scripts/ccp4.setup-sh \n"

            cmds += "giant.quick_refine {} {} {} params={} \n".format(os.path.join(out_dir,xtal_name,"multi-state-model.pdb"),
                                                           os.path.join(out_dir,xtal_name,"refine.mtz"),
                                                           os.path.join(out_dir,xtal_name,"*.cif"),
                                                           os.path.join(out_dir,xtal_name,"multi-state-restraints.refmac.params"))
