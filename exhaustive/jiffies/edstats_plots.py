...
def plot_NUDT22():
    ...
    compound_dirs = [
        os.path.join(NUDT22_dir, compound_dir)
        for compound_dir in os.listdir(NUDT22_dir)
...
def plot_DCP2B():
    ...
    FMOPL000435a_df = FMOPL000435a_df.rename(
        index=str, columns={"ES_occ": "es_occupancy", "Occupancy": "occupancy"}
    )

    params = master_phil.extract()
    params.output.out_dir = "/dls/science/groups/i04-1/elliot-dev/Work/exhaustive_search_data/DCP2B_18_09_20_exhaus"

    occupancy_histogram_with_exhaustive_search(
        FMOPL000435a_df, protein_name="DCP2B", compound="FMOPL000435a", params=params
    )
