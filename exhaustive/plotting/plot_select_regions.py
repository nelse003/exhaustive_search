...
def plot_protein_region(
    ...
    convex_hull_points = convex_hull_from_states(
    ...
    lig_atom_points = atom_points_from_sel_string(
        pdb=params.input.pdb,
    ...
    box_points = get_occupancy_group_grid_points(
        pdb=params.input.pdb,
        bound_states=bound_states,
        ground_states=ground_states,
        params=params,
    ...
    per_residue_convex_hull_points = convex_hull_per_residue(
        pdb=params.input.pdb,
        bound_states=bound_states,
        ground_states=ground_states,
        params=params,
    ...
    all_atoms = atom_points_from_sel_string(
        pdb=params.input.pdb, selection_string="all"
    )
    ...
    plt.savefig(
        filename=os.path.join(params.output.out_dir, "region_selected.png"), dpi=300
    )
