...
def repeat_validate(params):
    ...
    atom_points = atom_points_from_sel_string(
        ...
        selection_string=params.exhaustive.options.atom_points_sel_string,
    )

    lig_grid_points = convex_hull_grid_points(atom_points, params)

    plot_protein_and_selection(
        pdb=params.input.pdb,
        atom_points=lig_grid_points,
        plot_filename=os.path.join(params.output.out_dir, "lig_grid_points.png"),
        params=params,
    )
    ...
    per_residue_points = convex_hull_per_residue(
        pdb=params.input.pdb,
    ...
    plot_protein_and_selection(
        ...
        plot_filename=os.path.join(params.output.out_dir, "per_residue_points.png"),
        params=params,
    )
    ...
    for buffer_dist in np.arange(0, 1, 0.5):
        ...
        buffered_points = convex_hull_from_states(
            pdb=params.input.pdb,
            bound_states=bound_states,
            ground_states=ground_states,
            params=params,
...
def summary_validation(params):
    ...
    normalised_min_b_fac = (
        np.array(min_b_facs) - u_iso_to_b_fac(params.exhaustive.options.lower_u_iso)
    ) / (
        u_iso_to_b_fac(params.exhaustive.options.upper_u_iso)
        - u_iso_to_b_fac(params.exhaustive.options.lower_u_iso)
    )

    normalised_b_fac = (
        np.array(b_facs) - u_iso_to_b_fac(params.exhaustive.options.lower_u_iso)
    ) / (
        u_iso_to_b_fac(params.exhaustive.options.upper_u_iso)
        - u_iso_to_b_fac(params.exhaustive.options.lower_u_iso)
    )
    ...
    with open(
        os.path.join(params.output.out_dir, "validation_summary.csv"), "wb"
    ) as validation_csv:
