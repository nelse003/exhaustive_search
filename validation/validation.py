...
from exhaustive.plotting.run_scatter_plot import scatter_plot
...
def start_validate_logging(params):
    ...
    log_path = os.path.join(
        params.output.log_dir, params.validate.output.log_name + log_time
    )
    logger = logging.getLogger(__name__)
    hdlr = logger.FileHandler(log_path)
    formatter = logger.Formatter("%(asctime)s %(levelname)s \n %(message)s")
...
def check_validate_input_files(params):
    ...
    # Output directory
    try:
        assert os.path.exists(params.output.out_dir), "{} does not exist".format(
...
def occ_loop_merge_confs_simulate(params):
    ...
    logging.info(
        "Looping over simulated occupancies "
    ...
    for lig_occupancy in np.arange(
        ...
        if params.validate.options.overwrite or not os.path.exists(
            ...
            logging.info(
                "Using giant.merge_conformations to generate a pdb "
                "f with bound state occupancy {}".format(str(1 - lig_occupancy))
            )
            ...
            logging.info(
                "Skipping generating merged pdb"
        ...
        if params.validate.options.set_b is not None:
            ...
            if params.validate.options.set_all_b is not None:
                merged_pdb = (
                    merged_file_name + params.validate.output.set_all_b_name_extension
                )
            else:
                merged_pdb = (
                    merged_file_name + params.validate.output.set_b_name_extension
                )
        ...
        if params.validate.options.overwrite or not os.path.exists(simulated_mtz):
            ...
            logging.info(
                "Skipping the generation of simulated data:"
                "\n{}\n using fmodel as it already exists,"
                " for occupancy {}, and "
                " params.validate.options.overwrite is "
                "{}".format(
                    simulated_mtz, lig_occupancy, params.validate.options.overwrite
                )
            )
        try:
            assert os.path.exists(
        ...
        if params.validate.options.overwrite or not os.path.exists(
            ...
            if params.validate.options.use_qsub:
                ...
                cmd = (
                    "$CCP4/bin/ccp4-python /dls/science/groups/i04-1/"
                    ...
                    "exhaustive.options.generate_mtz={}"
                ).format(
                    merged_pdb,
                    merged_pdb + ".mtz",
                    params.output.out_dir,
                ...
                logging.info(
                    "Writing {} to run exhaustive" "search via qsub".format(sh_file)
                )

                with open(os.path.join(params.output.out_dir, sh_file), "w") as f:

                    f.write("#!/bin/bash\n")
                    f.write(
                        'export XChemExplorer_DIR="/dls/science/'
        ...
        if params.validate.options.generate_ccp4:
            if (
                ...
                logging.info(
                    ...
                    "\n{}\n to difference map .ccp4 f".format(merged_pdb + ".mtz")
                )
...
def run(params):
    ...
    logging.info(
        "Checking for files existence : " "wait for jobs submitted to the cluster"
    )
    ...
    logging.info(
        "This plots exhaustive search results, to confirm whether "
    ...
    logging.info(
        "Plotting occupancy, bfactor and mean |Fobs-Fcalc| "
